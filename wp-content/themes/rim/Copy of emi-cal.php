...
$tp =0;
function getEmi($t){
	global $i,$upto, $totalint, $rate,$monthly,$payment_date, $arr,$_SESSION,$tp,$x,$time;
    $i++;
    if($upto<=0){
        return 0;
    }
...
<tr>
    ...
    <td>
	<?php
		if(isset($_POST['new_principle']) && isset($_POST['left_time']) && $_POST['new_principle'] && $_POST['left_time'] && $i==$_POST['left_time']){
			$vals=$_POST['new_principle'];
		}else{
			$vals=number_format(round($e));
		}
	?>
        Rs. <input id="end_balnc_<?=$i;?>" class="inputKeyup" data-time="<?=$i;?>" type="text" value="<?php echo $vals;  ?>" disabled /><button data-id="end_balnc_<?=$i;?>" class="btn btnClick btn-green">Edit</button>
    </td>
</tr>
<?php
if(isset($_POST['new_principle']) && isset($_POST['left_time']) && $_POST['new_principle'] && $_POST['left_time']){
